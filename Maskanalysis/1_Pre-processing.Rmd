...
Update: Most of the notes have been examined separately.
...
Incorrect and problematic trials were removed ~~(N=272, 4.01%)~~ (N= 186, 2.77%)
We will analyse wrong trials separately.

Remaining trials: **6539**

```{r export error data}
error <- subset(data, data$ACC!=1)
write.csv(error,"220117_unfiltered_error.csv")
```
...
table(data$RTcut)
data = subset(data, select = -RTcut)
```

Unnatural reaction times below 0ms and above 3000ms were removed since they indicate insufficient attention (N=29; Fairs & Strijkers allow 2000ms for picture naming; Vogt, Hauber, Kuhlen & Rahman allowed 3000ms for picture naming from onset; Kessler, Treiman & Mullennix allowed 2000ms for word reading). 
...
```{r save}
write.table(data, "220117_full_data_cleaned.txt", sep="\t", row.names=FALSE)
```
