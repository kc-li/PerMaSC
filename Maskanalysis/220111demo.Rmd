...
library(gridExtra)
library(patchwork)
intertheme <- theme(axis.text.x = element_text(size = 12), # for x label text
        axis.text.y = element_text(size = 12), 
        axis.title.x = element_text(size = 15), # for x axis title
        axis.title.y = element_text(size = 15),
        legend.text = element_text(size=15), # for legend size
        legend.title = element_text(size=15))
singletheme <- theme(axis.text.x = element_text(size = 12), # for x label text
        axis.text.y = element_text(size = 12), 
        axis.title.x = element_text(size = 15), # for x axis title
        axis.title.y = element_text(size = 15),
        legend.text = element_text(size=15), # for legend size
...
497-486 # 11 visual gain when acoustically no mask
526-495 # 31 visual gain when acoustically masked
...
```{r plot function block - individual no average}
RT_plot_individual <- function(data, xaxis){
  ...
  stat_summary(fun =mean, geom="line", size = 1, aes(group =GROUP,linetype = GROUP)) + #1
  theme_classic() +
  ...
  legend.title = element_text(size=13)# for legend title
  )+
...
ggsave("figure/220213_individual_effect.png", p.combine, dpi = 300, width = 180, height = 108, units = "mm") #10:6
```

```{r plot function block - interaction no average}
...
ggsave("figure/220213_interaction_combine.png", p.combine, dpi = 300, width = 180, height = 108, units = "mm")
...
p1_predict <- ggplot(data, aes(PREDICTABILITY, RT)) +
  ...
  stat_summary(fun = mean, geom = "point", size = 5,aes(group = 1),colour=('#666666')) +
  stat_summary(fun =mean, geom="line", size = 3,aes(group = 1),colour=('#666666')) +
  labs(x = "Cloze Probability", y = "Mean Reaction Times in ms",linetype = "Group") +
  singletheme +
  coord_cartesian(ylim = c(350, 700))
...
#           align = "hv",
#           widths = c(2, 2, 2))
# ggsave("figure/220203_individual_effect.png", width = 10, height = 6)
...
p.combine
p.combine <- patchwork::patchworkGrob(p.combine)
ggsave("figure/220203_individual_effect.png", p.combine, width = 10, height = 6)
```
...
p1_acoustic_visual <- ggplot(data, aes(VISUAL, RT)) +
  ...
  labs(x = "Visual", y = "Mean Reaction Times in ms",linetype = "Group",colour = "Acoustic") +
  theme_classic() +
...
p1_acoustic_predict <- ggplot(data, aes(PREDICTABILITY, RT)) +
  ...
  stat_summary(fun = mean, geom = "point", size = 5,aes(group = ACOUSTIC,colour = ACOUSTIC),position = position_dodge(0.1)) +
  stat_summary(fun =mean, geom="line", size = 3,aes(group = ACOUSTIC,colour = ACOUSTIC),position = position_dodge(0.1)) +
  labs(x = "Cloze Probability", y = "Mean Reaction Times in ms",linetype = "Group",,colour = "Acoustic") +
  theme_classic() +
  intertheme+ # for legend title) 
  coord_cartesian(ylim = c(350, 700)) +
  scale_color_manual(values=c('#666666', "#CC0066"))
...
#           align = "hv",
#           widths = c(1, 1))
# ggsave("figure/220203interaction_combine.png",width = 10, height = 6)
...
p.interact.combine
p.combine <- patchwork::patchworkGrob(p.interact.combine)
ggsave("figure/220203_interaction_combine.png", p.combine, width = 10, height = 6)
```
...
ggplot(subset(data, CONDITION == 1|CONDITION ==4), aes(BLOCK,RT)) +
  ...
  theme(axis.text.x = element_text(size = 12), # for x label text
        axis.title.x = element_text(size = 15), # for x axis title
        axis.title.y = element_text(size = 15),
        legend.text = element_text(size=12), # for legend size
        legend.title = element_text(size=12))+ # for legend title) 
  coord_cartesian(ylim = c(340, 780))
ggsave("figure/220203_RTblock_subset.png", width = 8, height = 5)

```

```{r}
ggplot(subset(data, CONDITION == 1|CONDITION ==4), aes(TRIAL,RT)) +
  stat_summary(fun = mean, geom = "point",aes(group = interaction(GROUP,type),colour = type), size = 3, position = position_dodge(0.1)) +
  stat_summary(fun =mean, geom="line", size = 0.6, aes(group = interaction(GROUP,type), colour = type, linetype = GROUP), position = position_dodge(0.1)) +
  theme_classic()+
  labs(x = "Block", y = "Mean Reaction Times in ms",linetype = "Group",colour = "Mask On/Off") +
  ...
  theme(axis.text.x = element_text(size = 12), # for x label text
        ...
        legend.title = element_text(size=12))+ # for legend title) 
  coord_cartesian(ylim = c(340, 780))
ggsave("figure/220205_Trial_subset.png", width = 8, height = 5)


ggplot(subset(data, (CONDITION == 1|CONDITION ==4)&(BLOCK ==1|BLOCK ==4)), aes(TRIAL,RT)) +
  stat_summary(fun = mean, geom = "point",aes(group = type,colour = type), size = 3, position = position_dodge(0.1)) +
  stat_summary(fun =mean, geom="line", size = 0.6, aes(group = type, colour = type), position = position_dodge(0.1)) +
  theme_classic()+
  labs(x = "Block", y = "Mean Reaction Times in ms",linetype = "Group",colour = "Mask On/Off") +
  scale_colour_manual(labels = c("+Acoustic Mask, \n+Visual Mask","-Acoustic Mask,\n -Visual Mask"),values=c('#666666', "#CC0066")) +
  theme(axis.text.x = element_text(size = 12), # for x label text
