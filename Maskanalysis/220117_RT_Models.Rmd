...
# data <- mutate(data, AGE_c = AGE - mean(AGE, na.rm = TRUE)) ####this doesn't work anymore! why?
...
```{r summary table}
RTsummary_group <- groupwiseMean(RT ~ GROUP + CONDITION + PREDICT,
                                  data   = data,
                                  conf   = 0.95,
                                  digits = 3)
RTsummary_group
RTsummary <- groupwiseMean(RT ~ CONDITION + PREDICT,
                                  data   = data,
                                  conf   = 0.95,
                                  digits = 3)
RTsummary
write.csv(RTsummary_group, "220119RTsummary_group.csv", row.names = FALSE)
write.csv(RTsummary, "220119RTsummary.csv", row.names = FALSE)

groupwiseMean(RT ~ AUDIO,
                                  data   = data,
                                  conf   = 0.95,
                                  digits = 3)
511-491
groupwiseMean(RT ~ VIDEO,
                                  data   = data,
                                  conf   = 0.95,
                                  digits = 3)
511-491
groupwiseMean(RT ~ PREDICT,
                                  data   = data,
                                  conf   = 0.95,
                                  digits = 3)
541-462
groupwiseMean(RT ~ GROUP,
                                  data   = data,
                                  conf   = 0.95,
                                  digits = 3)
594-413
...
Random effects:
 ...
 ITEM     (Intercept)  7124.4   84.41              
          VIDEO1        470.7   21.69   -0.10      
          AUDIO1        928.8   30.48    0.03 -1.00
 SUBJECT  (Intercept) 83738.0  289.38              
          PREDICT1      285.8   16.91   0.24       
 Residual             42527.3  206.22              
Number of obs: 5653, groups:  ITEM, 234; SUBJECT, 52

Fixed effects:
                  Estimate Std. Error         df t value Pr(>|t|)    
(Intercept)      503.36646   40.61603   53.97621  12.393  < 2e-16 ***
AGE_c             -5.71522    3.24668   51.99084  -1.760 0.084234 .  
PREDICT1         -40.98438    6.61496  188.23199  -6.196 3.57e-09 ***
VIDEO1           -13.18042    3.18852  306.22890  -4.134 4.61e-05 ***
AUDIO1           -12.69010    3.43572  235.58972  -3.694 0.000275 ***
TRIAL_c           -0.64091    0.08081 5298.83830  -7.931 2.62e-15 ***
VIDEO1:AUDIO1      6.88519    2.88459 5436.51834   2.387 0.017026 *  
PREDICT1:AUDIO1    7.60838    3.31251  287.37782   2.297 0.022346 *  
AGE_c:PREDICT1    -0.63566    0.29204   47.42943  -2.177 0.034523 *  
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1

Correlation of Fixed Effects:
            (Intr) AGE_c  PREDICT1 VIDEO1 AUDIO1 TRIAL_ VIDEO1: PREDICT1:
AGE_c        0.024                                                       
PREDICT1     0.082  0.002                                                
VIDEO1      -0.007  0.001  0.000                                         
AUDIO1       0.000  0.000  0.010   -0.264                                
TRIAL_c      0.000  0.001 -0.005   -0.003 -0.001                         
VIDEO1:AUDI  0.000  0.000  0.000   -0.010 -0.001 -0.012                  
PREDICT1:AU  0.002 -0.001 -0.009   -0.009 -0.024 -0.006  0.002           
AGE_:PREDIC  0.004  0.151  0.004   -0.004 -0.004 -0.002  0.000   0.005   
...
Note: In an exploratory fashion, when AGE_c:PREDICT:AUDIO is added to the full model, the three-way interaction does not survive model reduction. 
...
```{r mask_data}
mask_data <- subset(data, data$CONDITION != 'aNMvM')
mask_data <- subset(mask_data, mask_data$CONDITION != 'aMvNM')
...
contrasts(mask_data$BLOCK) <- contr.sum(4)
...
```
```{r}
tab_model(lmm.all.optimized,lmm.adults.optimized,lmm.children.optimized)

tab_model(lmm.all.optimized,lmm.adults.optimized,lmm.children.optimized, pred.labels = c("Intercept","Age","Predictability","Visual Mask", "Acoustic Mask","Trial Order","Acoustic * Visual", "Acoustic * Predictability", "Age * Predictability"),
          dv.labels = c("All Subjects","Adults", "Children"),
          file = "figure/220120threeregressions.html")
webshot("figure/220120threeregressions.html", "figure/220120threeregressions.png")

#tab_model(lmm.all.optimized,lmm.adults.optimized,lmm.children.optimized,
          pred.labels = c("Intercept", "Acoustic Mask","Visual Mask","Predictability","Trial Order","Acoustic * Visual", "Acoustic * Predictability", "Age","Age * Predictability"),
          dv.labels = c("Adults", "Children","All Subjects"),
          file = "figure/220120threeregressions.html")
