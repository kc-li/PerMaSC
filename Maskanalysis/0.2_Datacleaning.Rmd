...
write.csv(combine,"datafile/secondbatch.csv",row.names = FALSE)
...
table <- table(fulldata$audio, fulldata$video, fulldata$context)
margin.table(table,3)
...
# Delete trials noted in 'Missing trials'
```{r}
mis <- read.csv("datafile/missing_trials.csv", header = T)
mis <- subset(missing, missing$Missing !="NONE")

fulldata <- fulldata %>%
  filter(!parid %in% mis$parid | !original_label %in% mis$Missing)
```
