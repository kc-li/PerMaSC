...
finalfilename$ = soundname$ +".txt"

writeFileLine: finalfilename$, "label", tab$, "correctans",tab$, "note", tab$, "duration_raw", tab$, "doublecheck"
...
for i from 1 to num_of_points
  ...
  if pointlabel$ == "check"
	...
	doublecheck$ = Get label of interval... 5 interval
    start = Get starting point... 1 interval
    duration_raw = confirmed_point - start - empty_start
    appendFileLine: finalfilename$, label$, tab$, correctans$, tab$, note$, tab$, duration_raw, tab$, doublecheck$
  endif
endfor
