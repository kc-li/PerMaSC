...
children <- subset(data, data$GROUP == 'Children') #3054
...
summary(lmm.all)
```
```{r, full start model - age}
lmm.all.age <- lmerTest::lmer(RT ~ 
                            AUDIO:VISUAL +
                            AUDIO:AGE +
                            VISUAL:AGE +
                            AUDIO:PREDICT +
...
```{r step - age}
step_object_age<-step(lmm.all.age, 
                  ddf = "Satterthwaite", 
                  alpha.random = 0.1, 
                  alpha.fixed = 0.05, 
                  reduce.fixed = TRUE, 
                  reduce.random = TRUE)
stepmodel.age<-get_model(step_object_age)
summary(stepmodel.age)
```
...
```
```{r random slope, echo=FALSE}
# only converges without AUDIO as slope
# PREDICT*GROUP not significant
lmm.all.optimized.slope <- lmerTest::lmer(RT ~ 
                            AUDIO:VISUAL +
                            AUDIO:PREDICT +
                            GROUP:PREDICT +
...
allFit(lmm.all.optimized.slope)
tab_model(lmm.all.optimized.slope)
```
```{r}
check_model(lmm.all.optimized)
```
