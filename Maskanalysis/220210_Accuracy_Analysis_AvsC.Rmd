...
authors: "Jasper Sim Hong" ; "Julia Schwarz"
date: "10/02/2022"
...
```{r}
# re-testing video:predict with random slopes since was marginal before
glm.11 <- glmer(ACC ~ 
                            #AUDIO:VIDEO + #drop
                            #AUDIO:PREDICT + #drop
                            #GROUP:PREDICT + drop
...
glm.12 <- glmer(ACC ~ 
                           ...
                           calc.derivs=FALSE), 
                           na.action=na.omit)
anova(glm.11, glm.12) #not sig. --> drop video:predict
```
...
tab_model(glm.optimized, pred.labels = c("Intercept","Acoustic Mask","Visual Mask","Cloze Probability","Age Group","Visual Mask*Age Group"),
          file = "figure/220213ACCfull.html")
          webshot("figure/220213ACCfull.html", "figure/220213ACCfull.png")
```
...
# Post-hoc comparisons of the video:group interaction
...
anova(glm.simple, glm.sub) #not sig.
anova(glm.simple, glm.item) #not sig.
anova(glm.simple,glm.complex) #not sig.
```

Testing fixed structure one more time with intercepts only model:
...
glm.a6 <- glmer(ACC ~ 
                           ...
                           calc.derivs=FALSE), 
                           na.action=na.omit)
anova(glm.a5, glm.a6) #not sig.


```


### *Final Adults Accuracy model:* old model had random slopes predict and video
...
### *Final Children Accuracy model:* as with the adult model, no random slopes included
...
tab_model(glm.final, glm.final.c, pred.labels = c("Intercept","Acoustic Mask","Visual Mask","Cloze Probability"),
          ...
          file = "figure/220211ACCreg.html")
          webshot("figure/220211ACCreg.html", "figure/220211ACCreg.png")
...
# Jasper's old analysis

```{r}
# this is glm.complex 
glm.fullfix <- glmer(ACC ~ 
                            AUDIO:VIDEO +
                            AUDIO:PREDICT +
                            GROUP:PREDICT + 
                            AUDIO:GROUP +
                           ...
                           calc.derivs=FALSE), 
                           na.action=na.omit)
drop1(glm.fullfix, test="Chisq")

glm.optimised_jsh <- glmer(ACC ~ 
                            VIDEO:GROUP + #keep
                           ...
                           calc.derivs=FALSE), 
                           na.action=na.omit)
tab_model(glm.optimised_jsh)
# posthoc for video:group

emmeans(glm.optimised_jsh, pairwise~VIDEO*GROUP)
```



*old* full data accuracy model before optimising under random slope consideration:
```{r}
glm.old <- glmer(ACC ~ 
                            VIDEO:GROUP + #keep
                            VIDEO:PREDICT + #keep
