...
RTassist.final <- RTassist.final %>%
  mutate(
    ...
    onset = str_sub(correctans,1,1),
    onset = as.factor(onset),
    onset.type = case_shen(
      onset == "b" ~ "plosive",
...
```{r characterize onset type}
levels(RTassist.final$onset)
RTassist.final %>%
  filter(onset=="c") %>%
  select(correctans)
```
...
nrow(RTassist.final[RTassist.final$diff.inten <10 & RTassist.final$diff.inten > -10,])/nrow(RTassist.final) #64.27%
...
```{r writedata}
write.table(RTassist.final,"interspeech/220308RTassist_final.txt", sep = "\t", row.names = FALSE)
```
